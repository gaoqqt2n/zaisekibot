...
fontpass = 'C:\Windows\Fonts\msgothic'
Ppass = u'C:\\Users\sldl77\Pictures\slackbot.jpg'
...
font = ImageFont.truetype(fontpass, int(tate/100*9))      #在室・学内フォントの設定（使用するパスの場所,サイズ）
draw.text((int(yoko/5), int(tate/100)), u'在室　　　　　　学内', font=font, fill='#FFF')      #在室・学内の描画（座標, テキスト,　フォント, 色）

font = ImageFont.truetype(fontpass, int(tate/100*8))     #メンバーのフォントの設定

rw=dbReaderWriter.ReaderWriter()
rw.ConnectToDB()

Fill = '#000'   #在室、学内メンバーの色
sugucolor = '#FF9100'   #すぐ戻るメンバーの色

sugumodoru=rw.GetSugumodoruUsers()  #すぐ戻るメンバーの取得

m = 0           #すぐ戻るメンバーの数を数える変数
for row in sugumodoru:     #すぐ戻るメンバーの数を数えるループ
    m+= 1
...
sugumodoru=rw.GetSugumodoruUsers()  #すぐ戻るメンバーの取得
for row in sugumodoru:
    if m <= 8:          #8人以内なら
        draw.text((int(yoko/150*31), int(tate/100*i)), row[0], font=font, fill=sugucolor)        #すぐ戻るメンバーの表示（座標,テキスト,フォント,色）
        i += tate/100   
    else:               #9人以上なら
        if j%2 == 0:    #偶数列の表示
            draw.text((int(yoko/150*12), int(tate/100*i)), row[0], font=font, fill=sugucolor)        #すぐ戻るメンバーの表示（座標,テキスト,フォント,色）

        else:           #奇数列の表示
            draw.text((int(yoko/150*40), int(tate/100*i)), row[0], font=font, fill=sugucolor)        #すぐ戻るメンバーの表示（座標,テキスト,フォント,色）
            i += tate/100 
        j += 1
...
m = 0           #メンバーの数を数える変数
for row in gakunai:     #メンバーの数を数えるループ
    m+= 1
...
canvas.save(Ppass, 'JPEG', quality=100, optimize=True)         #パスの場所に画像を保存 
